<template>
  <ul :class="classContainer" dir="rtl">
    <li
      v-for="(node, index) in nodes"
      :key="`node-${index}`"
      :class="{
        'nav-main-heading': node.heading,
        'nav-main-item br-b-menu': !node.heading,
    ...
    >
      ...
      >
        <i v-if="node.icon" :class="`nav-main-link-icon Fs4 ${node.icon}`"></i>
        <span v-if="node.name" class="nav-main-link-name text-right pr-2 Fs2 text-white" >{{ node.name }}</span>
